 ...
 */
 ...
 int AD5933::getByte(byte address, byte *value) {
     ...
     //Wire.setClock(400000); // fast mode, added Mar. 12 2025
     Wire.write(ADDR_PTR);
     Wire.write(address);
 ...
 bool AD5933::getComplexData(int *real, int *imag) {
     ...
     {
       //Serial.print("waiting for complex data: ");
       //Serial.println(i);
       i++;
     }
 ...
 bool AD5933::frequencySweep(int real[], int imag[], int n) {
     ...
     while ((readStatusRegister() & STATUS_SWEEP_DONE) != STATUS_SWEEP_DONE) {
         ...
         Serial.print("----------------------------------Frequency Sweep Iteration: ");
         Serial.println(i);
         i++;
  ...
  * @return Success or failure
  */
 bool AD5933::calibrate(float gain[], int phase[], int ref, int n) {
     int *real = new int[n];
     int *imag = new int[n];
     ...
     for (int i = 0; i < n; i++) {
         ...
         if (magnitude > 0.0 && ref > 0) {
             float refValue = static_cast<float>(ref);
             float denominator = refValue * magnitude;
  ...
  */
  /* OMITTED
 bool AD5933::calibrate(float gain[], int phase[], int real[], int imag[],
                        int ref, int n) {
